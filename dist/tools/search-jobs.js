...
export const searchJobsSchema = z.object({
    ...
    area: z
        .string()
        .optional()
        .describe("Location code. Common: 6001001000=台北市, 6001002000=新北市, 6001005000=台中市, 6001008000=高雄市"),
    ro: z
        .number()
        .int()
        .min(0)
    ...
    edu: z
        .string()
        .optional()
        .describe("Education level: 1=高中以下, 2=高中, 3=專科, 4=大學, 5=碩士, 6=博士"),
    remoteWork: z
        .number()
        .int()
        .min(1)
    ...
    s9: z
        .string()
        .optional()
        .describe("Experience (comma-separated): 1=應屆畢業, 2=1年以下, 3=1-3年, 4=3-5年, 5=5-10年, 6=10年以上"),
});
export async function searchJobs(params) {
    ...
    for (const job of jobs) {
        ...
        const salary = job.salaryLow && job.salaryHigh
            ...
            : "面議";
        lines.push(`[${shortCode}] ${job.jobName}`, `  Company: ${job.custName}`, `  Location: ${job.jobAddrNoDesc}`, `  Salary: ${salary}`, `  Posted: ${job.appearDate}`, `  Applied: ${job.applyCnt} people`, job.remoteWorkType > 0 ? `  Remote: ${job.remoteWorkType === 2 ? "Full" : "Partial"}` : "", "");
    }
    lines.push(`Use get_job_detail with job code (e.g. "${jobs[0]?.link.job.split("/").pop()}") for full details.`);
