...
export async function searchCompanies(params) {
    ...
    for (const co of companies) {
        lines.push(`[${co.encodedCustNo}] ${co.name}`, `  Industry: ${co.industryDesc}`, `  Location: ${co.areaDesc}`, `  Employees: ${co.employeeCountDesc}`, `  Capital: ${co.capitalDesc}`, `  Open jobs: ${co.jobCount}`, co.profile ? `  About: ${co.profile.slice(0, 100)}...` : "", "");
    }
    lines.push(`Use get_company_detail with company code (e.g. "${companies[0]?.encodedCustNo}") for full profile.`);
